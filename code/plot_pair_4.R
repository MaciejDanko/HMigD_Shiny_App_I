...
setModelResRaw<-function(n)
  switch(paste(n), 
         ...
         # '17' = raw_int_flows_b3,
         # '18' = raw_int_flows_f3,
         # '19' = raw_int_flows_n3a,
         # '20' = raw_int_flows_n3b,
         # '21' = raw_int_flows_n3c,
         # '22' = raw_int_flows_n3d,
         # '23' = raw_int_flows_n4a,
         # '24' = raw_int_flows_n4b,
         # '25' = raw_int_flows_n4c,
         # '26' = raw_int_flows_n4d
  )
...
tn<-function(k) as.numeric(as.character(k)); na2zero<-function(x) {x[is.na(x)]<-0;x}; na2one<-function(x) {x[is.na(x)]<-1; x}
...
prepare_data <- function(Data_input, orig, dest, flow = "pred", 
  ...
  true_accuracy.I<-rsm.data$A4_I_3.r_array[orig,dest,]
  true_accuracy.E<-rsm.data$A4_E_3.r_array[orig,dest,]
  # An<-length(table(c(Data_input$A_E.r,Data_input$A_I.r)))
  ...
  # add missing years to imem
  missyear<-Years[!Years %in% IMEM$year]
  if (length(missyear)){
    emptydf<-data.frame(q75=NA, p90=NA, origin2=orig,dest2=dest,year=missyear, p10=NA, q25=NA, median=NA)
    IMEM<-rbind(emptydf,IMEM)
    IMEM<-IMEM[order(IMEM$year),]
  }  
  limitLFS<-lfslim[dest][[1]]
  limitLFS<-full_join(limitLFS[limitLFS$YEAR %in% Data_input$year,],data.frame(YEAR=sort(unique(Data_input$year))),by='YEAR')
  limitLFS<-limitLFS[order(limitLFS$YEAR),]
  test_b<-limitLFS$wlimit_b>Data_input$s_rec
  test_a<-limitLFS$wlimit_a>Data_input$s_rec
  r_lfs <- Data_input$s_rec
  
  test_b[is.na(test_b)]<-TRUE
  test_b[is.na(r_lfs)]<-FALSE
  set.seed(which(dest==Countries)+100*which(orig==Countries))
  if (paste(any(test_b))=='TRUE') #r_lfs[test_b] <- exp(rnorm(length(r_lfs),log(r_lfs),mean(log(r_lfs),na.rm=TRUE)*0.01))[test_b]
    r_lfs[test_b] <- rtruncnorm(length(r_lfs[test_b]), a=0.001, b=limitLFS$wlimit_b[test_b], mean = r_lfs[test_b], sd = r_lfs[test_b]*0.1)
  #print('PREPARE')
  #print(An)
  An=3
  #print(na2one(na2zero((An+1)-Data_input$A_E.r)/An))
  res<-data.frame(sen=Data_input$orig, rec=Data_input$dest, year=Data_input$year,
             r_sen=Data_input$r_sen, 
             r_rec=Data_input$r_rec, 
             r_lfs=r_lfs, 
             test_a = test_a,
             test_b = test_b,
             pch_sen = na2one(pch.values[c('8-12','0-2','3','6','P')[tn(Data_input$I_S)+1]]),
             pch_rec = na2one(pch.values[c('8-12','0-2','3','6','P')[tn(Data_input$I_R)+1]]), # immigration by rec
             pch_lfs = na2one(rep(unname(pch.values['8-12']),nrow(Data_input))),
             alpha_sen=na2one(na2zero((An+1)-true_accuracy.E)/An),   #er  
             alpha_rec=na2one(na2zero((An+1)-true_accuracy.I)/An), #now: 3=high, 2=med, 1=low # ir
             alpha_lfs=na2one(max.alfa.lfs*(4-c(4,1,2)[na2zero(Data_input$a_imm.s)+1])/3), #now 3=high, 2=low
             cex_rec = na2one((5-Data_input$U_I.r)*(max.cex.rec-1)/4+1) ,  # in-mig reported by receiving
             cex_sen = na2one((5-Data_input$U_E.r)*(max.cex.sen-1)/4+1),
             cex_lfs = na2one((1-Data_input$LUI.s)*(max.cex.lfs-1)+1) ,
  ...
  cat('success',res$sen,res$rec,'\n')
  res
}

prepare_data_save <- function(Data_input, orig, dest, flow = "pred") {
  cat('prepare_data_save')
  Data_input <- as.data.frame(Data_input)
  true_accuracy.I<-rsm.data$A4_I_3.r_array[orig,dest,]
  true_accuracy.E<-rsm.data$A4_E_3.r_array[orig,dest,]
  Data_input<- Data_input[(Data_input$orig==orig) & (Data_input$dest == dest),]
  
  res<-data.frame(sending=Data_input$orig, receiving=Data_input$dest, year=Data_input$year,
             # r_sen=Data_input$r_sen, 
             # r_rec=Data_input$r_rec, 
             # r_lfs=Data_input$s_rec, 
             Flow05=unname(Data_input[,paste0(flow,"_q05")]), 
             ...
             'accuracy sending' = c('high','medium','low')[true_accuracy.E],
             'accuracy receiving' = c('high','medium','low')[true_accuracy.I],
...
prepare_aggregated_data <- function(Data_input, Data_input2, orig, dest, flow = "pred", na.rm=FALSE) {
  ...
  if (FALSE) Data_input2<-Data_input<-raw_int_flows_b
  ...
  missyear<-Years[!Years %in% IMEM$year]
  if (length(missyear)){
    z<-expand.grid(orig=orig,dest=dest, year=missyear)
    emptydf<-data.frame(q75=NA, p90=NA, origin2=z$orig,dest2=z$dest,year=z$year, p10=NA, q25=NA, median=NA)
    IMEM<-rbind(emptydf,IMEM)
    IMEM<-IMEM[order(IMEM$origin2,IMEM$dest2, IMEM$year),]
  }
...
get_aggregated <- function( cntr_sen_list=c('RO','BG'),
  ...
  MO1F<-setModelResRaw(m1)
  MO2F<-setModelResRaw(m2)
...
plot_aggregated <- function(cntr_sen_list=c('RO','BG'),
  ...
  M1<-M1[M1$year<=maxYear,]; M2<-M2[M2$year<=maxYear,]
  if (na.rm){
  ...
  if (linetype==3){
    XOUT<- seq(min(M1$year),max(M1$year), length.out=100)
    XOUT2<-seq(min(M2$year),max(M2$year), length.out=100)
    M1L<-spline(M1$year, M1$Flow50/1000, xout=XOUT)
    M2L<-spline(M2$year, M2$Flow50/1000, xout=XOUT2)
    M1L25<-spline(M1$year, M1$Flow25/1000, xout=XOUT)
    M2L25<-spline(M2$year, M2$Flow25/1000, xout=XOUT2)
    M1L75<-spline(M1$year, M1$Flow75/1000, xout=XOUT)
    M2L75<-spline(M2$year, M2$Flow75/1000, xout=XOUT2)
    M1L05<-spline(M1$year, M1$Flow05/1000, xout=XOUT)
    M2L05<-spline(M2$year, M2$Flow05/1000, xout=XOUT2)
    M1L95<-spline(M1$year, M1$Flow95/1000, xout=XOUT)
    M2L95<-spline(M2$year, M2$Flow95/1000, xout=XOUT2)
    if (showIMEM){
      IMEMrange<-M1$year[!is.na(M1$IMEM50)]
      y.to.NA<-function(x, XOUT) {x$y[(XOUT<min(IMEMrange))|(XOUT>max(IMEMrange))]<-NA; x}
      IMEM50 <- y.to.NA(spline(M1$year, M1$IMEM50/1000, xout=XOUT), XOUT)
    }
    ...
    XOUT<- seq(min(M1$year),max(M1$year)+1, length.out=100)
    XOUT2<-seq(min(M2$year),max(M2$year)+1, length.out=100)
    M1L<-approx(M1$year, M1$Flow50/1000, xout=XOUT, method = 'constant',rule=2)
    M2L<-approx(M2$year, M2$Flow50/1000, xout=XOUT2, method = 'constant',rule=2)
    M1L25<-approx(M1$year, M1$Flow25/1000, xout=XOUT, method = 'constant',rule=2)
    M2L25<-approx(M2$year, M2$Flow25/1000, xout=XOUT2, method = 'constant',rule=2)
    M1L75<-approx(M1$year, M1$Flow75/1000, xout=XOUT, method = 'constant',rule=2)
    M2L75<-approx(M2$year, M2$Flow75/1000, xout=XOUT2, method = 'constant',rule=2)
    M1L05<-approx(M1$year, M1$Flow05/1000, xout=XOUT, method = 'constant',rule=2)
    M2L05<-approx(M2$year, M2$Flow05/1000, xout=XOUT2, method = 'constant',rule=2)
    M1L95<-approx(M1$year, M1$Flow95/1000, xout=XOUT, method = 'constant',rule=2)
    M2L95<-approx(M2$year, M2$Flow95/1000, xout=XOUT2, method = 'constant',rule=2)
    if (showIMEM){
      IMEMrange<-M1$year[!is.na(M1$IMEM50)]
      y.to.NA<-function(x, XOUT) {x$y[(XOUT<min(IMEMrange))|(XOUT>max(IMEMrange+1))]<-NA; x}
      IMEM50<-y.to.NA(approx(M1$year, M1$IMEM50/1000, xout=XOUT, method = 'constant',rule=2), XOUT)
    }
    ...
    XOUT<- seq(min(M1$year),max(M1$year), length.out=100)
    XOUT2<-seq(min(M2$year),max(M2$year), length.out=100)
    M1L<-approx(M1$year, M1$Flow50/1000, xout=XOUT, method = 'linear')
    M2L<-approx(M2$year, M2$Flow50/1000, xout=XOUT2, method = 'linear')
    M1L25<-approx(M1$year, M1$Flow25/1000, xout=XOUT, method = 'linear')
    M2L25<-approx(M2$year, M2$Flow25/1000, xout=XOUT2, method = 'linear')
    M1L75<-approx(M1$year, M1$Flow75/1000, xout=XOUT, method = 'linear')
    M2L75<-approx(M2$year, M2$Flow75/1000, xout=XOUT2, method = 'linear')
    M1L05<-approx(M1$year, M1$Flow05/1000, xout=XOUT, method = 'linear')
    M2L05<-approx(M2$year, M2$Flow05/1000, xout=XOUT2, method = 'linear')
    M1L95<-approx(M1$year, M1$Flow95/1000, xout=XOUT, method = 'linear')
    M2L95<-approx(M2$year, M2$Flow95/1000, xout=XOUT2, method = 'linear')
    if (showIMEM){
...
get_ymax_raw <- function(cntr_sen='PL',
  ...
  # print('ymax')
  cat('get_ymax_raw')
  if (length(cntr_sen) && length(cntr_rec) && cntr_sen!=cntr_rec && length(m1) && length(m2)){
    MO1<-setModelRes(m1) 
    MO2<-setModelRes(m2)
...
plot_models <- function(cntr_sen='MT',
                        cntr_rec='GR',
                        m1=1,
                        m2=2,
                        MO1=NULL,
  ...
  if (length(cntr_sen) && length(cntr_rec) && cntr_sen!=cntr_rec && length(m1) && length(m2)){
    test_IMEM <- (cntr_sen %in% c('HU','MT','PT','GR')) & (cntr_rec %in% c('HU','MT','PT','GR'))
    if (m1==m2) col1<-col2<-'black'
    #print(setYmax)
    #print(Ymax)
    ##print(ls())
    ...
    plot_LFS <- (as.integer(m1)%in%models_with_LFS) |  (as.integer(m2)%in%models_with_LFS)
    ...
    if (linetype==3){
      ...
      M2L95<-spline(M2$year, M2$Flow95/1000, xout=XOUT2)
      if (showIMEM){
        if (IMEMonly) XOUT<-XOUT[XOUT<=2008] else XOUT<-XOUT[XOUT<=min(2019,maxYear)]
        IMEMrange<-M1$year[!is.na(M1$QFlow50)]
        y.to.NA<-function(x, XOUT) {x$y[(XOUT<min(IMEMrange))|(XOUT>max(IMEMrange))]<-NA; x}
        IMEM50 <- y.to.NA(spline(M1$year, M1$QFlow50/1000, xout=XOUT), XOUT)
        IMEM10 <- y.to.NA(spline(M1$year, M1$QFlow05/1000, xout=XOUT), XOUT)
        IMEM90 <- y.to.NA(spline(M1$year, M1$QFlow95/1000, xout=XOUT), XOUT)
        IMEM25 <- y.to.NA(spline(M1$year, M1$QFlow25/1000, xout=XOUT), XOUT)
        IMEM75 <- y.to.NA(spline(M1$year, M1$QFlow75/1000, xout=XOUT), XOUT)
      }
      ...
      XOUT<- seq(min(M1$year),max(M1$year)+1, length.out=1000)
      XOUT2<-seq(min(M2$year),max(M2$year)+1, length.out=1000)
      M1L<-approx(M1$year, M1$Flow50/1000, xout=XOUT, method = 'constant',rule=2)
      M2L<-approx(M2$year, M2$Flow50/1000, xout=XOUT2, method = 'constant',rule=2)
      M1L25<-approx(M1$year, M1$Flow25/1000, xout=XOUT, method = 'constant',rule=2)
      M2L25<-approx(M2$year, M2$Flow25/1000, xout=XOUT2, method = 'constant',rule=2)
      ...
      if (showIMEM ){
        IMEMrange<-M1$year[!is.na(M1$QFlow50)]
        if (IMEMonly) XOUT<-XOUT[XOUT<=2009] else XOUT<-XOUT[XOUT<=min(2019,maxYear+1)]
        y.to.NA<-function(x, XOUT) {x$y[(XOUT<min(IMEMrange))|(XOUT>max(IMEMrange+1))]<-NA; x}
        IMEM50<-y.to.NA(approx(M1$year, M1$QFlow50/1000, xout=XOUT, method = 'constant',rule=2), XOUT)
        IMEM75<-y.to.NA(approx(M1$year, M1$QFlow75/1000, xout=XOUT, method = 'constant',rule=2), XOUT)
        IMEM25<-y.to.NA(approx(M1$year, M1$QFlow25/1000, xout=XOUT, method = 'constant',rule=2), XOUT)
        IMEM10<-y.to.NA(approx(M1$year, M1$QFlow05/1000, xout=XOUT, method = 'constant',rule=2), XOUT)
        IMEM90<-y.to.NA(approx(M1$year, M1$QFlow95/1000, xout=XOUT, method = 'constant',rule=2), XOUT)
      }
      ...
      if (showFit) if (m1!=m2) lines(M2L, lwd=lwd, col=adjustcolor(col2,alpha.f = 0.9))
      
      #lines(M1$year, M1$Flow50/1000, lwd=2, col=adjustcolor(col1,alpha.f = 0.8),type='s')
      #lines(M2$year, M2$Flow50/1000, lwd=2, col=adjustcolor(col2,alpha.f = 0.8),type='s')
    } else if (linetype==1){
      XOUT<- seq(min(M1$year),max(M1$year[!is.na(M1$Flow50)]), length.out=100)
      XOUT2<-seq(min(M2$year),max(M2$year[!is.na(M2$Flow50)]), length.out=100)
      M1L<-approx(M1$year, M1$Flow50/1000, xout=XOUT, method = 'linear')
      M2L<-approx(M2$year, M2$Flow50/1000, xout=XOUT2, method = 'linear')
      M1L25<-approx(M1$year, M1$Flow25/1000, xout=XOUT, method = 'linear')
      M2L25<-approx(M2$year, M2$Flow25/1000, xout=XOUT2, method = 'linear')
      ...
      if (showIMEM){
        if (IMEMonly) XOUT<-XOUT[XOUT<=2008] else XOUT<-XOUT[XOUT<=min(2019,maxYear)]
        IMEM50<-approx(M1$year, M1$QFlow50/1000, xout=XOUT, method = 'linear')
        IMEM25<-approx(M1$year, M1$QFlow25/1000, xout=XOUT, method = 'linear')
        IMEM75<-approx(M1$year, M1$QFlow75/1000, xout=XOUT, method = 'linear')
        IMEM10<-approx(M1$year, M1$QFlow05/1000, xout=XOUT, method = 'linear')
        IMEM90<-approx(M1$year, M1$QFlow95/1000, xout=XOUT, method = 'linear')
    ...
    if (plot_LFS) {
      lines(M1$year+0.5*(linetype==2), M1$r_lfs/1000, ylim=c(0,YLIM[2])/1000, 
                        col='white', 
                        bg = 'white', 
                        pch=M1$pch_lfs, 
    ...
    if (plot_LFS) {
      ...
      lines(M1$year+0.5*(linetype==2), M1$r_lfs/1000, ylim=c(0,YLIM[2])/1000, 
            col=1, 
            pch=c(NA,4)[1+M1$test_b], 
            cex=(M1$cex_lfs-1)/2+1, type='p')
    }
    
    #print('SEND')
    #print(M1$alpha_sen)
    # print(M1$year+0.5*(linetype==2))
    # print(M1$r_sen/1000)
    # print(sapply(M1$alpha_sen, function(a) adjustcolor(2,alpha.f = a, red.f = 0.8, green.f = 0.8, blue.f = 0.8)))
    # print(sapply(M1$alpha_sen, function(a) adjustcolor(2,alpha.f = a)))
    # print(M1$pch_sen)
    # print((M1$cex_sen-1)/2+1)
    #print('REC')
    #print(M1$alpha_rec)
    # print(M1$year+0.5*(linetype==2))
    # print(M1$r_rec/1000)
    # print(sapply(M1$alpha_rec, function(a) adjustcolor(2,alpha.f = a, red.f = 0.8, green.f = 0.8, blue.f = 0.8)))
    # print(sapply(M1$alpha_rec, function(a) adjustcolor(2,alpha.f = a)))
    # print(M1$pch_rec)
    # print((M1$cex_rec-1)/2+1)
    # test<-data.frame(x=M1$year+0.5*(linetype==2),
    #            y=M1$r_rec/1000,
    #            col=sapply(M1$alpha_rec, function(a) adjustcolor(2,alpha.f = a, red.f = 0.8, green.f = 0.8, blue.f = 0.8)),
    #            bg=sapply(M1$alpha_rec, function(a) adjustcolor(2,alpha.f = a)),
    #            pch=M1$pch_rec,
    #            cex=(M1$cex_rec-1)/2+1
    # )
    # save(list='test',file='test.rda')
    # print('END')
    ...
    x0<-max(sapply(c(letters,LETTERS),strwidth))
    y0<-max(sapply(c(letters,LETTERS),strheight))
    if (linetype==2) facx<-(length(XLIM[1]:XLIM[2])+1)/length(XLIM[1]:XLIM[2]) else facx<-1
    if (plotLegend) {
      #rect(usr[2],usr[3],usr[2]+max(sapply(c(letters,LETTERS),strwidth))*10,usr[4],xpd=TRUE,lwd=1.5, col=0, border=1)
      ...
      if (An==3) {
        ...
        text(usr[2]+((0:4)*leg.spread+x0*leg.tab)*facx ,rep(usr[4] - y0*2.6, 5), c('v.low','low','med','high','v.high'), xpd=TRUE, cex=leg.cex, adj=c(0.5,0))
        lines(usr[2]+((0:4)*leg.spread+x0*leg.tab)*facx ,rep(usr[4] - y0*4, 5),
              col=sapply((1:5)/5, function(a) adjustcolor(2,alpha.f = a, red.f = 0.8, green.f = 0.8, blue.f = 0.8)), 
              bg = sapply((1:5)/5, function(a) adjustcolor(2,alpha.f = a)), 
              pch=20, xpd=TRUE, type='p', cex=3)
        lines(usr[2]+((0:4)*leg.spread+x0*leg.tab)*facx ,rep(usr[4] - y0*6, 5),
              col=sapply((1:5)/5, function(a) adjustcolor(4,alpha.f = a, red.f = 0.8, green.f = 0.8, blue.f = 0.8)), 
              bg = sapply((1:5)/5, function(a) adjustcolor(4,alpha.f = a)), 
              pch=20, xpd=TRUE, type='p', cex=3)
      }
      if (plot_LFS) lines(usr[2]+((0:1)*leg.spread+x0*leg.tab)*facx ,rep(usr[4] - y0*8,2),
                          ...
                          pch=20, xpd=TRUE, type='p', cex=3)
      if (showISO2){
        sleg <- paste(cntr_sen,'(sending)')
        rleg <- paste(cntr_rec,'(receiving)')
        lfsleg <- paste(cntr_rec,'(LFS, receiving)')
      } else {
        sleg <- 'Sending country'
        rleg <- 'Receiving country'
        lfsleg <- 'Receiving country (LFS)'
      }
      text(usr[2]+((An-0.5)*leg.spread+x0*leg.tab)*facx ,usr[4] - y0*4, sleg, cex=leg.cex, adj=0, xpd=TRUE)
      text(usr[2]+((An-0.5)*leg.spread+x0*leg.tab)*facx ,usr[4] - y0*6, rleg, cex=leg.cex, adj=0, xpd=TRUE)
      if (plot_LFS) text(usr[2]+((An-0.5)*leg.spread+x0*leg.tab)*facx ,usr[4] - y0*8, lfsleg, cex=leg.cex, adj=0, xpd=TRUE)
      ...
      if (plot_LFS) {
        ...
        lines(usr[2]+(2*leg.spread+x0*leg.tab)*facx ,usr[4] - y0*(28.5+sy),
              col= 1, 
              pch= 4, xpd=TRUE, type='p', 
              cex=rev((1-(0:1))*(max.cex.lfs-1))/2+1 )
        text(usr[2]+((0:1)*leg.spread+x0*leg.tab)*facx ,rep(usr[4] - y0*(27.25+sy),2), c('high','low'), xpd=TRUE, cex=leg.cex, adj=c(0.5,0))
        text(usr[2]+(2.25*leg.spread+x0*leg.tab)*facx ,usr[4] - y0*(28.8+sy),'Perturbed restricted records', xpd=TRUE, cex=leg.cex, adj=c(0,0))
      }
      ...
      if (showIMEM && IMEMlegend) if (m1==m2) { 
        legend('topleft', bty = 'n', col = c(col2,adjustcolor(col3,alpha.f = 0.9),adjustcolor(col3,alpha.f = 0.9,green.f = 0.4)), lwd = 3, lty = 1, legend = c('HMigD model','IMEM','QuantMig'))
      } else {
        legend('topleft', bty = 'n', col = c(col1,col2,adjustcolor(col3,alpha.f = 0.9),adjustcolor(col3,alpha.f = 0.9,green.f = 0.4)), lwd = 3, lty = 1, legend = c('HMigD model 1','HMigD model 2','IMEM','QuantMig'))      
      }
...
plot_src_summary<-function (data){
  srcpalette<- c(EUROSTAT='green3',UN='blue2',QuantMig='orange2',DESTATIS='red2',UK_NSO='yellow',IMEM='cyan3',Unknown='white')
  srcpalette<-srcpalette[names(data)]
  v <- names(data)
  v[v=='DESTATIS']<-'DE:DESTATIS'
  v[v=='UK_NSO']<-'UK:NSO'
  names(data) <- v
...
plot_Sources<-function(direction='E',year=2002,cexx=1, cexy=1, lox=1, loy=1,NoDataTxt='Missing data') {
  ...
  DAAY<-DAA[,,paste(year)]
  diag(DAAY)<-' '
  colors<-c('black','green3','blue2','orange2','red2','yellow','cyan3','white')
  DAAY[is.na(DAAY)]<-''
  LEVELS[LEVELS=='NA']<-''
  ...
  #if (length(namat)) {
  v <- c(LEVELS[-1],NoDataTxt)
  v[v=='DESTATIS']<-'DE:DESTATIS'
  v[v=='UK_NSO']<-'UK:NSO'
  
  l<-legend(length(cn)+lox,length(rn)+loy,xpd=TRUE,bty='n',fill=c(colors[-1],NA), border = NA, legend = v)
  legend(l$rect$left, min(l$text$y)+abs(min(diff(l$text$y)))*1.45,'',pch=4,bty='n',xpd=TRUE,cex=1.45)
}  
...
extend_array<-function(original_array, sending = TRUE){
  dim_original_array <- dim(original_array)
  dim_original_names <- dimnames(original_array)
  a <- dim_original_array[1]
  y <- dim_original_array[2]
  b <- a
  new_array <- array(NA, dim = c(a, b, y))
  for (i in 1:a) {
    if (sending) new_array[, i, ] <- original_array else new_array[i , , ] <- original_array
  }
  dimnames(new_array)<-list(dim_original_names[[1]],dim_original_names[[1]], dim_original_names[[2]])
  new_array
}

plot_Accuracy<-function(direction='E',adjusted=FALSE, showNA=TRUE, cexx=1, cexy=1, lox=1, loy=1) {
  #if (adjusted) ACU<-rsm.data_updated else 
  #ACU<-rsm.data
  
  #rsm.data$A4_I_3.r_array[,'DE',]
  LEVELS<-1:5#sort(paste(unique(as.vector(c(ACU$A_I.r,ACU$A_E.r)))))
  if (!adjusted) {
    if (direction=='E') DAA<-ACU$A_E_3.r_base else DAA<-ACU$A_I_3.r_base
  } else{
    if (direction=='I') {#DAA<-ACU$A_E_3.r else DAA<-ACU$A_I_3.r
      DAA<-apply(rsm.data$A4_I_3.r_array,c(2,3),function(x) paste(unique(x),collapse = '&',sep=''))
    } else {
      DAA<-apply(rsm.data$A4_E_3.r_array,c(1,3),function(x) paste(unique(x),collapse = '&',sep=''))
    }
  }
  if (direction=='I') {#DAA<-ACU$A_E_3.r else DAA<-ACU$A_I_3.r
    DAA['DE',]<-'5'
  ...
  if (direction=='I') {
    colors<-c('green4','orange','red3','#AA000060','#00AA0060') 
    legend<-c('High','Medium','Low','Low (QuantMig)','High (*)','Missing data')
  } else {
    colors<-c('green4','orange','red3','#00AA0060') 
    legend<-c('High','Medium','Low','High (*)','Missing data')
  }
    #   legend<-c('High','Medium','Low','Low (QuantMig\n estimates)','High (low for flows to DK)','Missing data')
    # }
  # } else {
  #   # colors<-c('#006000','#009B00','#FFA500','#DD0000','#8B0000')
  #   colors <- rev(brewer.pal(n = 5, name = "Spectral"))
  #   legend<-c('Very high','High','Medium','Low','Very low','Missing data')
  # } 
  ...
  #if (length(namat)) {
  l<-legend(length(cn)+lox,length(rn)+loy,xpd=TRUE,bty='n',fill=c(colors,NA), border = NA, legend = legend)
  legend(l$rect$left, min(l$text$y)+abs(min(diff(l$text$y)))*1.45,'',pch=4,bty='n',xpd=TRUE,cex=1.45)
}  
